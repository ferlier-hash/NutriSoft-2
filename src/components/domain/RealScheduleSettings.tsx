...
export function RealScheduleSettings({organizationId}:{organizationId:string}) {
 ...
 {loading?<p role="status">Cargando horarios…</p>:loaded&&<form onSubmit={event=>void save(event)} className="scroll-pb-28"><fieldset disabled={busy} className="min-w-0 space-y-6">
 ...
 <div className="sticky bottom-[calc(3.5rem+env(safe-area-inset-bottom))] z-10 -mx-4 -mb-4 border-t border-border-subtle bg-surface/95 px-4 py-3 backdrop-blur sm:bottom-0"><Button type="submit" className="w-full sm:w-auto">{busy?'Guardando…':'Guardar horarios y reglas'}</Button></div></fieldset></form>}
