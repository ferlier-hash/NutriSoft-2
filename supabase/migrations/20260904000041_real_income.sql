ALTER TABLE app.appointment_payment_movements ADD COLUMN refunded_payment_id uuid REFERENCES app.appointment_payment_movements(id), ADD COLUMN request_id uuid UNIQUE;
CREATE VIEW api.income_movements WITH(security_invoker=true) AS SELECT id,appointment_id,amount,currency,movement_kind,method,note,occurred_at,refunded_payment_id FROM app.appointment_payment_movements WHERE NOT security.is_current_platform_admin();
GRANT SELECT ON api.income_movements TO authenticated;
CREATE FUNCTION api.record_income_movement(p_appointment uuid,p_request uuid,p_amount numeric,p_date date,p_method text,p_refund uuid DEFAULT NULL,p_note text DEFAULT NULL) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
...
BEGIN
 ...
 IF FOUND THEN
 IF previous.appointment_id<>p_appointment OR previous.recorded_by<>auth.uid() OR previous.amount<>p_amount OR previous.refunded_payment_id IS DISTINCT FROM p_refund OR previous.method<>p_method OR previous.occurred_at::date<>p_date OR previous.note IS DISTINCT FROM nullif(trim(p_note),'') THEN RAISE EXCEPTION 'Solicitud reutilizada con datos diferentes.'; END IF;
